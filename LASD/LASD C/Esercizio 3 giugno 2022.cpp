/*
Scrivere una funioen che, dati due grafi G e H, verifichi se H è un sottografo di G.
...
int is_subgraph(graph *G1, graph *G2){
	...
	while(i < n_v){
		...
		//Se il numero di archi del sottografo G1 è maggiore del grafo G2, allora G1 non è un sottografo di G2
		if(count1 > count2){//Qui il problema riguarda la situazione specchiata, perché il controllo effettuato precedentemente considera un sottografo anche grafi con liste di
							//adiacenza con più elementi es° G1: 1 -> 2   G2: 1. In questo caso il controllo precedente considera G1 sottografo di G2 sbagliando.
			is_subgraph = 0;//Quindi oltre agli elementi si controlla la lunghezza della lista di adiacenza, in  quanto un grafo con una lista di adiacenza con più archi
							//non può mai essere sottografo di un grafo con meno archi
		}
		i++;
	}
